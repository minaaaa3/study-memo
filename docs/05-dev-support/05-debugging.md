...
<Step title="再現する">
問題が起きる手順を確認。毎回同じように再現できるか？
</Step>

<Step title="絞り込む">
どこで問題が起きているか特定。フロント？バック？DB？
</Step>

<Step title="原因を特定">
なぜ問題が起きているか調査。ログやデバッガーを活用。
</Step>

<Step title="修正する">
正しい動作に直す。最小限の変更を心がける。
</Step>

<Step title="検証する">
本当に直ったか確認。元の問題と新しい問題が出ていないか。
</Step>
...
```mermaid
flowchart TD
    A[問題発生] --> B{ブラウザのNetworkタブでAPIレスポンス確認}
    B -->|正しい| C[フロントエンドの問題]
    B -->|おかしい| D{バックエンドのログ確認}
    D -->|SQLの結果が正しい| E[バックエンドの問題]
    D -->|SQLの結果がおかしい| F[DBクエリの問題]
...
<WhyButton title="なぜアヒルに話すの？">

**ラバーダック・デバッグ**は実際に効果があるテクニックです。

問題を誰か（またはゴム製のアヒル）に説明すると：

「えーと、ここでユーザーを取得して...あ、ここでnullチェックしてないじゃん」

説明する過程で、自分で問題に気づくことが多いです。話す相手がいないときは、独り言でもOK。
